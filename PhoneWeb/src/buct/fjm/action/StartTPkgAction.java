...
@Component ("startTPkg")
@Scope ("prototype")
public class StartTPkgAction extends ActionSupport{
	...
	public String execute() throws Exception
	{
		...
		while((temp=br.readLine())!=null){
		  if(temp.startsWith("StartTime")){
			  sb.append("StartTime="+format.format(new Date())).append(FileOperator.SYSTEM_SEPARATOR);
			  continue;
		  }
		  if(temp.startsWith("IsInput")){
			  sb.append("IsInput="+true).append(FileOperator.SYSTEM_SEPARATOR);
			  continue;
		  }
		  if(temp.startsWith("Truncate")){
			  sb.append("Truncate="+true).append(FileOperator.SYSTEM_SEPARATOR);
			  continue;
		  }
